...
| End-to-end tests (Playwright) | ✅ | `preview_flow.spec.ts` (11) + `linked_view.spec.ts` (8) – 19 tests, all pass. Needs dev server on **:5175** (config `baseURL`) and `npx playwright install chromium`. The linked-view specs stub the backend from a captured response, so they need no analysis run. Not run in CI. |
