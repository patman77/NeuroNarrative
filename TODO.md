...
**Stages 1-6 are built** (2026-08-02); stages 7-8 are design only, and both are blocked on labels that have not been made yet. `docs/status.md` is the living current-state snapshot, including what is unproven.
...
| Design docs | ✅ | `docs/phenomena-detection-design.md` (L0–L4 pipeline, rules → classical ML → DL, calibration, evaluation) and `docs/session-narrative-design.md` (BK3 grammar, function roles, the report). Both carry an "as built" section recording where the design proved wrong |
| **Stage 1** — channel resolver, LP domain | ✅ | `phenomena/conditioning.py` + the mirrored resolver in `gsrParser.ts`; five-step resolution order, median-interval time units, device `Tag Number` ingestion. Golden fixture asserted by both suites. Reference recording: `data+baseline`, 9.2e-5 LP, 0.05 s for 161k samples; legacy event count **23 → 64** |
| **Stage 2** — L1 primitives + A/T/BE/LPA/LPD/LPB | ✅ | `phenomena/primitives.py` (tonic/phasic split, hysteresis legs with stall timeout and onset/peak trimming) + `detectors/{deflection,discharge,level}.py` + `fusion.py`. Reference recording: A 115, T 183, BE 34, LPA_slow 19, LPB 1.52 LP, LPD 3.7 A/min |
| **Stage 3** — stimulus locking, `X`, `KVZ` | ✅ | `detectors/stimulus.py` + `services/protocol.py`. 191 locked / 179 unlocked, 19 `X`, 30 `KVZ`. Unlocked lowers confidence, never filters — in solo it usually just means the operator was quiet |
| **Stage 3** — BK3 cue inventory, solo + observed phrasings | ✅ | `services/protocol.py` `CUE_INVENTORY`, with an explicit "observed in practice, NOT in BK3" group and stem-tolerant matching for ASR damage ("ruft ihr" → "Ruf dir"). Instruction turns: 13 → **29 of 303** |
| **Stage 4** — calibration + artefact masking | ✅ | `calibration.py` + `detectors/artefact.py`. `lp_offset`/`a_unit_lp` optional on `/api/analyze` and available in the UI; **no charge zone is named without an offset**. Movement is reported as `KB`, not silently masked |
| **Stage 5** — stable ids, labelling, `Tag Number` | ✅ | Content-derived ids in `phenomena/schema.py`; `services/labels.py` + `/api/labels/*` + verdict buttons. Four verdicts, content-hash recording identity, atomic writes. **No labels have been made yet** — the bottleneck is effort, not tooling |
| **Stage 6** — evaluation harness | ✅ | `phenomena/evaluate.py` + `POST /api/labels/{id}/evaluate`. Per-kind precision/recall/F1, ±2 s onset tolerance, `has_recall_evidence` so recall is never claimed from confirmations alone |
...
| Cross-panel linking | ✅ | Kind filters drive the timeline markings; hovering in any panel highlights and scrolls the other two, and scrolls the detail chart. The overview is a separate hover source from the detail chart, so pointing at a marker there travels the detail view to it — but only when it is not already on screen. `utils/smoothScroll.ts` — ease-in-out, capped at 2 s |
...
| Narrative §4.2 — topic rollup ranked by total discharge | ❌ | |
| **Stage 8** — deep learning | ❌ | Explicitly gated on ≥50 labelled sessions – see design §8 |
...
| Speaker diarisation | ❌ | **Deliberately not done.** The corpus is solo — one voice alternating between instruction and content — so there is nothing to separate. Cue matching in `services/protocol.py` carries the role signal instead |
...
| Protocol-cue fallback for silent stretches | ✅ | `protocol_segment` now delegates to `services/protocol.py`, which parses the verified BK3 cue inventory into a procedure/exercise tree. "Danke" is **not** a cue — it appears nowhere in either manual as an instruction — and widening is bounded by the tree rather than by a flat radius |
...
| Two GSR CSV parsers (frontend `gsrParser.ts` for the preview, backend `conditioning.py` for the analysis) picked **different columns** on a real export — the frontend plotted the 0.05-quantised `Baseline`, the backend analysed `Resistance(kOhm)` — so the preview and the analysis showed different signals. | `frontend/src/utils/gsrParser.ts`, `backend/app/services/phenomena/conditioning.py` | ✅ Fixed – both now implement the same documented resolution order and are pinned by the golden fixture `tests/fixtures/mindwalker_export.csv`, asserted from both sides. They still share no code, which is the standing risk. |
...
| **Cross-panel hover scrolled to the wrong place and appeared to highlight nothing.** `scrollChildIntoView` used `child.offsetTop`, which is relative to the nearest *positioned* ancestor; neither list is positioned, so the offset came from further up the tree. The row was highlighted the whole time but sat outside the scroll viewport — indistinguishable from no highlight, and no error either way. Compounded by the narrative matching the phenomenon *nearest a section's start*, which is usually outside the tolerance because sections open on a spoken cue and phenomena cluster later. | `frontend/src/utils/smoothScroll.ts`, `components/PhenomenaPanel.tsx` | ✅ Fixed – measured with `getBoundingClientRect`; `HoverTarget.endSec` carries the span so the first phenomenon *inside* a section is used. |
| `BE_MAX_RISE_TIME_SEC` (5.0 s) is unvalidated — the manual never quantifies "schnell". Sensitivity is tabulated in `detectors/discharge.py`: the count roughly doubles between 3 s and 5 s and plateaus after. The evaluation harness now exists; it needs **labels**, which have not been made. | `backend/app/services/phenomena/detectors/discharge.py` | Medium |
...
| Detail chart width is `duration × 80 px/s` — a 54-min recording produces a ~260 000 px wide SVG carrying up to 400 markers. Scrolls, but heavy. Needs windowing to the visible range. | `frontend/src/components/SignalPreview.tsx` | Medium |
...
| Speaker diarisation not implemented | — | ✅ Won't do – the corpus is solo, so there is nothing to separate; cue matching carries the role signal |
