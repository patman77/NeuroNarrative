...
| Session narrative (auto-Sitzungsbericht) | ✅ | `services/narrative.py` + `components/SessionNarrative.tsx`. One section per protocol segment with real times from the cues and real LP from the signal; markdown export. On the reference recording: 12 sections tiling 00:00–53:55, LP 6.00 → 4.79 |
| Phenomena in the UI | ✅ | Two-column scrollable layout (`.analysis-columns`): phenomena left, events+summaries right, each list scrolling independently under a bounded height. Whole rows are click-to-seek, and `handleSeek` opens a collapsed preview first so a click never silently no-ops. |
| Phenomena panel internals | ✅ | `components/PhenomenaPanel.tsx` – ranked "Größte Ladung zuerst", per-kind filter chips, click-to-seek, uncalibrated/artefact caveats shown. `EventTimeline` still shows the legacy list alongside |
