...
| Hover on a marker opens a tooltip with raw metrics and quick actions; selecting a row highlights the marker | The panels are linked instead of tooltipped: hovering a row in any of the three panels highlights and scrolls the other two and marks the moment in both charts, and a narrative section shades its whole span. A click seeks, keeps the mark after the pointer leaves, and repositions the page on the charts. Quick actions live on the rows, not in a tooltip. |
