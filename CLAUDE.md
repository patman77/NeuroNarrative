...
CI (`.github/workflows/ci.yml`) runs frontend lint + typecheck + build and backend pytest; `release.yml` builds and publishes the desktop app on a `v*` tag (see "Releases" below). CI does **not** run E2E — those need a dev server on 5175 and browser binaries, so run them locally when touching `SignalPreview`/`UploadPanel`.
...
- **Quit any running copy before smoke-testing.** The single-instance guard makes a second launch attach to the first and exit 0, so the test silently passes against the *old* build. `smoke_desktop.sh` greps the log for "already running"; so does the release workflow.
- **`torch` is excluded in the spec.** mlx-whisper declares it as a dependency but never imports it on the load-model/transcribe path, and PyInstaller was bundling 410 MB for nothing — 985 MB → 550 MB on Apple silicon. Verified by transcribing with it excluded, not by reading the imports. If a future mlx-whisper does start importing it, the smoke test's ASR check is what will catch it.

### Releases

`.github/workflows/release.yml` builds the bundle on macOS (arm64 + Intel), Windows and Linux for
every `v*` tag and attaches the archives, plus a `SHA256SUMS`, to a GitHub release.
`workflow_dispatch` builds the same artifacts without publishing — that is how to test a packaging
change before committing to a tag.
